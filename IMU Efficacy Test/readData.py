...
with open('data.csv', 'w', newline='') as file:
    ...
    while True:
        ...
        parts = line.split(' ')
        if len(parts) == 6:  # Check if line can be split into six parts
            try:
                x = float(parts[1])  # Try to convert parts to floats
                y = float(parts[3])
                z = float(parts[5])
                writer.writerow([x, y, z])  # Write row to CSV
        ...
        if data_points == n:
            # Read the CSV file
            df = pd.read_csv('data.csv', header=None, names=['X', 'Y', 'Z'])
